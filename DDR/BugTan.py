...
class controlBugTan():
    def __init__(self, ddr, qGoal, lidar):
        ...
        self. minDistance = 0
        self.folloDirection = -1
        self.oi = []


    def followOi(self):
        laser_nless_1 = self.lidar.getBoundary()[39]
        oi = []
        for laser in self.lidar.getBoundary():
            if abs(laser.distValue - laser_nless_1.distValue) > 100:
                if laser.distValue < laser_nless_1.distValue:
                    oi.append(laser)
                else:
                    oi.append(laser_nless_1)

            laser_nless_1 = laser
        for ois in oi:
            print ("oi =", ois.distValue)
    ...
    def folloWall(self):
        ...
        if not self.ddr.updating or self.dispacing:
            ...
            if self.folloDirection > 0:
                ...
                if self.lidar.getBoundary()[5].distValue < self.lidar.getBoundary()[1].distance -0:
                    self.sensor2 = True
                    self.lidar.getBoundary()[5].setColor([255,255,0])
                else:
                    self.lidar.getBoundary()[5].setColor([0,255,0])

                if self.lidar.getBoundary()[10].distValue < self.lidar.getBoundary()[2].distance -15:
                    self.sensor3 = True
                    self.lidar.getBoundary()[10].setColor([255,255,0])
                else:
                    self.lidar.getBoundary()[10].setColor([0,255,0])

                if self.lidar.getBoundary()[0].distValue <= self.lidar.getBoundary()[0].distance -25:
                    self.sensor1C = True
                    self.lidar.getBoundary()[0].setColor([255,0,0])

                if self.lidar.getBoundary()[5].distValue <= self.lidar.getBoundary()[1].distance -25:
                    self.sensor2C = True
                    self.lidar.getBoundary()[5].setColor([255,0,0])

                if self.lidar.getBoundary()[10].distValue <= self.lidar.getBoundary()[2].distance -25:
                    self.sensor3C = True
                    self.lidar.getBoundary()[10].setColor([255,0,0])
                ...
                if self.lidar.getBoundary()[35].distValue < self.lidar.getBoundary()[1].distance -0:
                    self.sensor2 = True
                    self.lidar.getBoundary()[35].setColor([255,255,0])
                else:
                    self.lidar.getBoundary()[35].setColor([0,255,0])

                if self.lidar.getBoundary()[30].distValue < self.lidar.getBoundary()[2].distance -15:
                    self.sensor3 = True
                    self.lidar.getBoundary()[30].setColor([255,255,0])
                else:
                    self.lidar.getBoundary()[30].setColor([0,255,0])

                if self.lidar.getBoundary()[0].distValue <= self.lidar.getBoundary()[0].distance -25:
                    self.sensor1C = True
                    self.lidar.getBoundary()[0].setColor([255,0,0])

                if self.lidar.getBoundary()[35].distValue <= self.lidar.getBoundary()[1].distance -25:
                    self.sensor2C = True
                    self.lidar.getBoundary()[35].setColor([255,0,0])

                if self.lidar.getBoundary()[30].distValue <= self.lidar.getBoundary()[2].distance -25:
                    self.sensor3C = True
                    self.lidar.getBoundary()[30].setColor([255,0,0])
            ...
            if self.sensor2C and self.sensor3 and not self.sensor1 and not self.sensor3C :
                self.stateMachine = self.FRW

        if self.stateMachine == self.ROT_IZQ:
            self.angleTarget = self.ddr.getAngle() - self.rotatingStep * np.pi/180 * self.folloDirection
